...
export default function Navbar() {
  ...
  const Unlogged = () => (
    <div className="pr-10 bg-white flex justify-center gap-x-6">
      <NavLink
        className="pt-1 text-md font-medium border border-input hover:bg-gray h-9 rounded-md px-3 text-black border-blue"
        to="/signup"
      >
        Daftar
      </NavLink>
      <NavLink
        className="pt-1 text-md font-medium bg-blue h-9 rounded-md px-3 text-white border-blue hover:bg-blue-80"
        to="/login"
      >
        Masuk
      </NavLink>
    ...
    return (
      <div className="relative flex flex-col justify-start items-center">
        ...
        >
          ...
          <img src="/assets/avatar.svg" alt="" className="size-10" />
        </div>
      </div>
    );
  };
    ...
    <header className="sticky top-0 z-[9999] bg-white">
      <nav className="flex justify-between items-center shadow-xl rounded-b-xl h-24 px-5">
        ...
        <div className="hidden lg:flex gap-x-10 items-center text-base">
          <button
            className="text-md font-semibold hover:text-blue"
            onClick={() => {
              navigate("/");
              setTimeout(
                () =>
        ...
        <i
          className="bx bx-menu block text-5xl cursor-pointer lg:hidden"
          onClick={() => setIsMenuOpen(!isMenuOpen)}
        ></i>

        {/* Mobile Menu */}
        <div
          className={`absolute top-24 left-0 w-full bg-white flex flex-col items-center gap-6 text-lg transform ${
            isMenuOpen
