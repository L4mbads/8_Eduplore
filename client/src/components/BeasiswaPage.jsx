...
export default function BeasiswaPage() {
  ...
  useEffect(() => {
    ...
    async function getBeasiswaList() {
      const response = await fetch(
        `${backendAPI}/beasiswa-management/beasiswa?name=${encodeURIComponent(
          ""
        )}`
      );
      if (!response.ok) {
  ...
  async function onSubmit(e) {
    ...
    const response = await fetch(
      `${backendAPI}/beasiswa-management/beasiswa?name=${encodeURIComponent(
        query.name
      )}&degree=${encodeURIComponent(query.degree)}&date=${encodeURIComponent(
        query.date
      )}`
    );
    if (!response.ok) {
  ...
  const Select = ({ value, options, onChange }) => {
    return (
      <select
        value={value}
        onChange={onChange}
        className="border border-blue border-2 rounded-xl py-2 mb-2 px-3 grow "
      >
        {options.map((option) => (
  ...
  function NoResult() {
    return (
      <>
        <h1 className="text-xl lg:text-5xl pt-16 h-96 text-blue font-semibold">
  ...
  return (
    <div className="relative">
      <img src="/assets/Vector 3.png" className="absolute bottom-0 w-full" />
      <div className="relative flex flex-col items-center">
        <div className="py-16 w-full bg-white">
          <h1
            className={`transition duration-1000 ease-in-out  select-none text-center text-blue font-semibold tracking-wide text-4xl
        ...
        <div className="bg-white shadow-xl w-full flex flex-col px-6 sticky top-24 z-[10000]">
          <div class="">
            <input
              ...
              id="name"
              type="name"
              placeholder="Cari nama beasiswa di sini..."
          ...
          <div className="flex flex-wrap gap-4">
            ...
            />
            <input
              class="shadow appearance-none border-2  rounded-xl grow py-2 mb-2 px-3 leading-tight focus:outline-none focus:shadow-outline border-blue"
              id="name"
              type="date"
              placeholder=""
            ...
            />
            <button
              className="transition-all shadow grow bg-blue hover:bg-blue-80 text-white font-bold py-2 px-4 mb-2 rounded-xl focus:outline-none focus:shadow-outline hover:tracking-widest tracking-wider"
              type="button"
              onClick={onSubmit}
            >
              Cari!
